...
class GridSampler(ParallelSampler):
    ...
    def config(self, varied_params, cosmo_names, cosmo_min, cosmo_fid, cosmo_max, nsample, nstep, filename):
        global grid_sampler
        grid_sampler = self

        self.converged = False
        self.nsample = nsample #self.read_ini("nsample_dimension", int, 1)
        self.save_name = filename #self.read_ini("save", str, "")
        self.nstep = -1 #nstep #self.read_ini("nstep", int, -1)
        self.allow_large = False #self.read_ini("allow_large", bool, False)
    ...
    def setup_sampling(self):
        ...
        #outer product
        total_samples = self.nsample**len(self.varied_params)
        ...
        self.sample_points = itertools.product(*[np.linspace(param[0], param[1], num=self.nsample) 
            for param in zip(self.params_min, self.params_max)])
